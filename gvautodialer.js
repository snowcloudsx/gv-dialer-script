...
(function () {
  ...
  function buildCSS(themeKey) {
    ...
    #gv-call-panel.hidden { opacity: 0; transform: translateY(-10px) scale(0.97); pointer-events: none; }
    #gv-call-panel-header {
      display: flex; align-items: center; justify-content: space-between;
      cursor: move; padding: 12px 14px; border-bottom: 1px solid var(--gv-border);
    }
    ...
    #gv-call-panel-body { padding: 14px; display: flex; flex-direction: column; gap: 12px; }
    #gv-call-panel-lead { font-size: 11px; line-height: 1.6; min-height: 30px; }
    #gv-call-panel-lead .gv-cpl-name { font-size: 14px; font-weight: 600; margin-bottom: 6px; }
    #gv-call-panel-lead .gv-cpl-row { color: var(--gv-muted); margin-bottom: 3px; }
    #gv-call-panel-lead .gv-cpl-val { color: var(--gv-text); }
    #gv-call-panel-outcomes {
      display: flex; gap: 8px; border-top: 1px solid var(--gv-border); padding-top: 12px;
    }
    .gv-cpo-btn {
  ...
  const PANEL_HTML = `
      ...
      <details class="gv-accordion">
        ...
        <div class="gv-accordion-body">
          <div style="font-size:11px;color:var(--gv-muted);line-height:1.6">
            Google Voice Glass Dialer v<span id="gv-settings-version">7.0.0</span><br>
    ...
    <div id="gv-call-panel" class="hidden">
      <div id="gv-call-panel-header">
        <div id="gv-cph-dot" style="width:7px;height:7px;border-radius:50%;background:var(--gv-accent);box-shadow:0 0 6px var(--gv-accent);flex-shrink:0"></div>
  ...
  function init() {
    ...
    function callWatcherTick() {
      ...
      if (hasCall) {
        if (dialerRunning || currentCallLead) return;
        let phone = '';
        try {
